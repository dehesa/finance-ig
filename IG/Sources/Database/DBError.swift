...
extension IG.Database.Error {
    ...
    /// A factory function for `.unknown` Database errors.
    /// - parameter message: A brief explanation on what happened.
    /// - parameter error: The underlying error that happened right before this error was created.
    /// - parameter suggestion: A helpful suggestion on how to avoid the error.
...
extension IG.Database.Error {
    ...
    internal struct Message: IG.ErrorNameSpace {
        ...
        static var  sessionExpired: Self { .init("The \(IG.Database.printableDomain) instance wasn't found") }
    ...
    internal struct Suggestion: IG.ErrorNameSpace {
        ...
        static var keepSession: Self { .init("The \(IG.Database.printableDomain) functionality is asynchronous; keep around the \(IG.Database.self) instance while a response hasn't been received") }
