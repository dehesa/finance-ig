        ...
        self._database.publisher { _ in "SELECT * FROM \(Database.Application.tableName)" }
            .read { (sqlite, statement, query, _) in
                ...
                while true {
                    ...
                    case let e: throw IG.Error(.database(.callFailed), "An error occurred querying a table for '\(Database.Application.self)'.", info: ["Error code": e])
                    }
                }
            }.mapError(errorCast)
            .eraseToAnyPublisher()
    ...
    public func update(applications: [API.Application]) -> AnyPublisher<Never,IG.Error> {
        self._database.publisher { _ in
            ...
            for app in applications {
                ...
                try sqlite3_step(statement).expects(.done) { IG.Error(.database(.callFailed), "An error occurred storing values on '\(Database.Application.self)'.", info: ["Error code": $0]) }
