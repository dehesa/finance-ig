...
extension Streamer.Request.Accounts {
    ...
    public func subscribeToConfirmations(account: IG.Account.Identifier, snapshot: Bool = true) -> AnyPublisher<Streamer.Deal,Streamer.Error> {
        let item = "TRADE:".appending(account.rawValue)
        let properties = [Streamer.Deal._Field.confirmations.rawValue]
...
fileprivate extension Streamer.Deal {
...
extension Streamer {
    ...
    public struct Deal {
        ...
        internal init(account: IG.Account.Identifier, item: String, update: Streamer.Packet) throws {
            ...
            do {
                self.confirmation = try decoder.decode(Confirmation.self, from: .init(confirmationString.utf8))
