...
extension Streamer.Request.Deals {
    ...
    /// Subscribes to the given account and receives updates on working orders.
//    public func subscribeToWorkingOrders(account: IG.Account.Identifier, snapshot: Bool = true) -> AnyPublisher<Streamer.WorkingOrder,Streamer.Error> {
//        let (item, field) = ("TRADE:".appending(account.rawValue), Streamer.Deal.Field.workingOrders.rawValue)
//        let decoder = JSONDecoder()
//
//        return self.streamer.channel
//            .subscribe(on: self.streamer.queue, mode: .distinct, item: item, fields: [field], snapshot: snapshot)
//            .tryCompactMap { (update) -> Streamer.WorkingOrder? in
//                guard let payload = update[field]?.value else { return nil }
//                do {
//                    return try decoder.decode(Streamer.WorkingOrder.self, from: .init(payload.utf8))
