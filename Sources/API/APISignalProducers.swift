...
extension SignalProducer where Error==API.Error {
    ...
    internal func request<T>(_ requestor: @escaping API.Request.Generator.Request<T>) -> SignalProducer<API.Request.Wrapper,API.Error> where Value==API.Request.ValidatedValues<T> {
        return self.attemptMap { [requestor] (validated) -> Result<API.Request.Wrapper,API.Error> in
            ...
            return .success((request, validated.api))
        }
    }
    ...
    internal func request<T>(_ method: API.HTTP.Method, _ relativeURL: String, version: Int, credentials usingCredentials: Bool, queries: API.Request.Generator.Query<T>? = nil, headers: API.Request.Generator.Header<T>? = nil, body: API.Request.Generator.Body<T>? = nil) -> SignalProducer<API.Request.Wrapper,API.Error>  where Value==API.Request.ValidatedValues<T> {
        return self.attemptMap { (validated) -> Result<API.Request.Wrapper,API.Error> in
            ...
            if let queryGenerator = queries {
                ...
                do {
                    urlQueries = try queryGenerator(validated.api, validated.values)
                } catch let error as API.Error {
                    return .failure(error)
                } catch let error {
            ...
            do {
                ...
                request.addHeaders(version: version, credentials: credentials, try headers?(validated.api, validated.values))
                
                if let bodyGenerator = body {
                    let blob = try bodyGenerator(validated.api, validated.values)
...
extension SignalProducer where Value==API.Request.Wrapper, Error==API.Error {
    ...
    internal func send(expecting type: API.HTTP.Header.Value.ContentType? = nil) -> SignalProducer<API.Response.Wrapper,API.Error> {
        return self.flatMap(.latest) { (urlRequest, api) -> SignalProducer<API.Response.Wrapper,Error> in
...
extension API {
    // TODO: Delete me once there is no further call to it
    internal func makeRequest(_ request: @escaping API.Request.Generator.SolicitationDeleteMe) -> SignalProducer<API.Request.Wrapper,API.Error> {
    ...
    internal func paginatedRequest<PT:Decodable,T>(request requestGenerator: @escaping API.Request.Generator.SolicitationDeleteMe, expectedStatusCodes: [Int]? = [200], decoder: @escaping API.Codecs.DecoderGenerator = API.Codecs.jsonDecoder, _ valueHandler: @escaping (_ api: API, _ page: Signal<PT,API.Error>.Value) -> ([Signal<T,API.Error>.Event],URLRequest?)) -> SignalProducer<T,API.Error> {
